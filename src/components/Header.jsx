...
import { GraduationCap, User, PlusCircle, Menu, X } from 'lucide-react';
  ...
  return (
    <>
      <header className="fixed top-0 left-0 right-0 z-50 flex justify-center px-4 pt-3 pb-2 transition-all duration-700">
        ...
        <div className={`w-full max-w-6xl transition-all duration-700 ease-out ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-3'}`}>
            ...
            <div className="flex items-center gap-3">
              ...
              {isLoggedIn ? (
                <div className="flex items-center gap-3 border-l border-slate-700/50 pl-3">
                  ...
                  >
                    <User size={18} className="text-[#c9a227] group-hover:scale-110 transition-transform" />
          ...
          {mobileMenuOpen && (
            <div className="md:hidden mt-4 mx-4 p-6 rounded-2xl bg-[#0a1628]/95 border border-[#c9a227]/20 backdrop-blur-2xl animate-in fade-in slide-in-from-top-4">
              <div className="flex flex-col gap-6 items-center">
                <Link to="/home" className="text-xs tracking-[0.3em] font-bold text-[#c9a227] uppercase">Home</Link>
                <Link to="/about" className="text-xs tracking-[0.3em] font-bold text-slate-400 uppercase">About</Link>
                <Link to="/contact" className="text-xs tracking-[0.3em] font-bold text-slate-400 uppercase">Contact</Link>
                {isLoggedIn && (
                  <button 
                    onClick={() => navigate('/register-event')}
      ...
      {modal === "login" && (
        <Modal title="Faculty Login" onClose={closeModal}>
